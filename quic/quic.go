...
func (c *Conn) Read(p []byte) (n int, err error) {
	...
	slog.Debug("Read from QUIC stream:", "nBytes", n, "localAddr", c.LocalAddr(), "remoteAddr", c.RemoteAddr(), "err", err)
	return
}

func (c *Conn) Write(p []byte) (n int, err error) {
	n, err = c.Stream.Write(p)
	slog.Debug("Wrote to QUIC stream:", "nBytes", n, "localAddr", c.LocalAddr(), "remoteAddr", c.RemoteAddr(), "err", err)
	return
}
